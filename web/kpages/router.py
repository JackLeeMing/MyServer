...
def load_handlers(handler_dir='restful', member_filter=None):
...
def _load_handlers(handler_dir='restful', member_filter=None):
    '''
...
def _sorted_hanlders(handlers):
    ...
    handlers = [(pattern, order, h) for h in handlers for pattern,
                order in h.__urls__]
    # handlers.sort(cmp=operator, key=lambda x: x[1])
